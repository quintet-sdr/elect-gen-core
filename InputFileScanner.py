...
class Window(Frame):
    ...
    def init_window(self):
        ...
        self.master.title("Input File Scanner - Student-course Allocation AI Tool")
    ...
    def runScan(self):
        ...
        if self.stuFile != None or self.supFile != None or self.keywordsFile != None:
            ...
            studentsError, coursesError = scanInputData(self.stuFile, self.supFile, self.keywordsFile)

            if studentsError == 0 and coursesError == 0:
                showinfo("Success", "No input errors have been found in both files. You're good to go!")
            else:
                showerror("Error",
                          "Looks like there a few inconsistencies in files provided. Certain keywords provided in the excel files don't match the taxonomy tree. The incosistencies have been marked for you, please open and check again.")
